...
RUN RELEASE_URL="https://api.github.com/repos/jesseduffield/lazygit/releases/latest" \
  && DOWNLOAD_URL="https://github.com/jesseduffield/lazygit/releases/latest/download/lazygit_"\
  && LAZYGIT_VERSION=$(curl -s "${RELEASE_URL}" | grep -Po '"tag_name": "v\K[^"]*') \
  && curl -Lo lazygit.tar.gz "${DOWNLOAD_URL}${LAZYGIT_VERSION}_Linux_x86_64.tar.gz" \
